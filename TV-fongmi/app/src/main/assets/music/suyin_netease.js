...
async function getTopLists() {
  const res = await axios_1.default.get("https://music.163.com/api/toplist", {
    headers: {
  ...
  for (const item of list) {
    ...
    const entry = { id: id, title: name, coverImgUrl: item.coverImgUrl || item.coverImageUrl || "", description: item.updateFrequency || "" };
    if (name.indexOf("云音乐") === 0 || name.indexOf("飙升") >= 0 || name.indexOf("新歌") >= 0 || name.indexOf("热歌") >= 0) {
      if (!currentGroup || currentGroup.title !== "热门榜单") { currentGroup = { id: "hot", title: "热门榜单", data: [] }; groups.push(currentGroup); }
      currentGroup.data.push(entry);
    } else if (name.indexOf("榜") >= 0) {
      if (!currentGroup || currentGroup.title !== "全部榜单") { currentGroup = { id: "all", title: "全部榜单", data: [] }; groups.push(currentGroup); }
      currentGroup.data.push(entry);
    }
  }
  if (groups.length === 0 && list.length > 0) {
    groups.push({ id: "all", title: "全部榜单", data: list.map(function (item) { return { id: String(item.id), title: item.name, coverImgUrl: item.coverImgUrl || "", description: item.updateFrequency || "" }; }) });
  }
  return groups;
}
...
async function getMediaSource(musicItem, quality) {
  try {
    const res = await axios_1.default.get(
      `http://oiapi.net/api/Music_163&id=${musicItem.id}`,
      {
        headers: {
    ...
    throw new Error(`获取音乐源失败: ${error.message}`);
  }
}
const headers = {
  authority: "music.163.com",
...
module.exports = {
  ...
  srcUrl: "https://raw.gitcode.com/Crystim/mfp/raw/main/%E6%BA%AF%E9%9F%B3%E7%BD%91%E6%98%93.js",
