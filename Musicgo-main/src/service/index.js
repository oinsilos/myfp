...
module.exports = async function () {
    ...
    RNTrackPlayer.addEventListener(Event.RemotePause, () => TrackPlayer.pause());
    RNTrackPlayer.addEventListener(Event.RemotePrevious, () => TrackPlayer.skipToPrevious());
    RNTrackPlayer.addEventListener(Event.RemoteNext, () => TrackPlayer.skipToNext());
    RNTrackPlayer.addEventListener(Event.RemoteDuck, async ({ paused, permanent }) => {
        ...
        const tempRemoteDuckConf = Config.getConfig("basic.tempRemoteDuck");
        if (tempRemoteDuckConf === "lowerVolume") {
            if (paused) {
                const tempRemoteDuckVolume = Config.getConfig("basic.tempRemoteDuckVolume") ?? 0.5;
