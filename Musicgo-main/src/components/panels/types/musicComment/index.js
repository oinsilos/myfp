...
export default function MusicComment(props) {
    const { musicItem } = props;
    const [reqState, comments, getMusicComments] = useComments(musicItem);
    const { t } = useI18N();
    const listBody = <FlashList ListFooterComponent={comments?.length ? <ListFooter state={reqState} onRetry={getMusicComments}/> : null} ListEmptyComponent={<ListEmpty state={reqState} onRetry={getMusicComments}/>} estimatedItemSize={100} renderItem={({ item }) => {
            return <Comment comment={item}/>;
        }} onEndReachedThreshold={0.1} onEndReached={() => {
    ...
    return (<PanelFullscreen>
            <VerticalSafeAreaView style={globalStyle.fwflex1}>
                ...
                <View style={styles.musicItemContainer}>
                    <FastImage style={styles.musicItemArtwork} source={musicItem?.artwork} placeholderSource={ImgAsset.albumDefault}/>
                    <View style={styles.musicItemContent}>
                        <ThemeText fontSize="subTitle" numberOfLines={1}>
                            {musicItem.title}
                        </ThemeText>
                        <ThemeText fontSize="description" numberOfLines={1} fontColor="textSecondary">
                            {musicItem.artist}
                        </ThemeText>
                    </View>
                </View>
