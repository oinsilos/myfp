...
export default function AddToMusicSheet(props) {
    const sheets = useSheetsBase();
    const { musicItem = [], newSheetDefaultName } = props ?? {};
    const safeAreaInsets = useSafeAreaInsets();
    const { t } = useI18N();
    return (<PanelBase renderBody={() => (<>
                    <PanelHeader hideButtons title={t("panel.addToMusicSheet.title", {
                count: Array.isArray(musicItem) ? musicItem.length : 1,
            })}/>
                    <View style={style.wrapper}>
                        <FlatList data={sheets ?? []} keyExtractor={sheet => sheet.id} style={{
                marginBottom: safeAreaInsets.bottom,
            }} ListHeaderComponent={<ListItem withHorizontalPadding key="new" onPress={() => {
                        ...
                        onCancel() {
                            showPanel("AddToMusicSheet", {
                                ...
                                newSheetDefaultName,
                                    ...
                                    <ListItem.ListItemImage fallbackImg={ImgAsset.add}/>
                                    <ListItem.Content title={t("panel.addToMusicSheet.newMusicSheet")}/>
                                </ListItem>} renderItem={({ item: sheet }) => (<ListItem withHorizontalPadding key={`${sheet.id}`} onPress={async () => {
                    try {
                        ...
                        Toast.warn(t("panel.addToMusicSheet.toast.fail"));
                                    ...
                                    <ListItem.Content title={sheet.title} description={t("panel.addToMusicSheet.count", {
