...
export default function AssociateLrc(props) {
    const { musicItem } = props;
    const [input, setInput] = useState("");
    const colors = useColors();
    const { t } = useI18N();
    return (<PanelBase keyboardAvoidBehavior="height" height={vmax(30)} renderBody={() => (<>
                    <PanelHeader title={t("panel.associateLrc.title")} onCancel={hidePanel} onOk={async () => {
            ...
            ]} placeholderTextColor={colors.textSecondary} placeholder={t("panel.associateLrc.inputPlaceholder")} maxLength={80}/>
                </>)}/>);
}
