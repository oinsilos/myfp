...
export default function EditMusicSheetInfo(props) {
    ...
    async function onConfirm() {
        // 判断是否相同
        if (coverImg === musicSheet?.coverImg &&
        ...
        if (coverImg && coverImg !== musicSheet?.coverImg) {
            newCoverImg = addFileScheme(`${pathConst.dataPath}sheet${musicSheet.id}${coverImg.substring(coverImg.lastIndexOf("."))}`);
            try {
    ...
    return (<PanelFullscreen>
            <VerticalSafeAreaView style={globalStyle.fwflex1}>
                ...
                <View style={style.row}>
                    ...
                    <TouchableOpacity onPress={onChangeCoverPress} onLongPress={() => {
            setCoverImg(undefined);
                ...
                <View style={style.row}>
                    ...
                    <Input numberOfLines={1} textAlign="right" value={title} hasHorizontalPadding={false} onChangeText={onTitleChange} style={{
            ...
            borderBottomColor: colors.text,
                ...
                <TouchableOpacity activeOpacity={0.6} onPress={onConfirm} style={[
            {
