...
export default function LyricListWrapper(props) {
    const hash = props.route.key;
    const dataStore = searchResultStore.useValue();
    return <LyricList data={dataStore.data[hash]}/>;
}
const ITEM_HEIGHT = rpx(120);
function LyricListImpl(props) {
    const data = props.data;
    const searchState = data?.state ?? RequestStateCode.IDLE;
    const { t } = useI18N();
    return searchState === RequestStateCode.PENDING_FIRST_PAGE ? (<Loading />) : (<FlashList estimatedItemSize={ITEM_HEIGHT} renderItem={({ item }) => (<LyricItem lyricItem={item} onPress={async () => {
                try {
            ...
            }}/>)} ListEmptyComponent={<ListEmpty state={searchState}/>} ListFooterComponent={data?.data?.length ? <ListFooter state={searchState}/> : null} data={data?.data}/>);
}
