...
export default function useSearchLrc() {
     ...
     * pluginHash: 搜索条件
     */
    const search = useCallback(async function (query, queryPage, pluginHash) {
        /** 如果没有指定插件，就用所有插件搜索 */
        ...
        if (plugins.length === 0) {
            searchResultStore.setValue(produce(draft => {
                draft.data = {};
            }));
            return;
        }
        ...
        plugins.forEach(async (plugin) => {
            ...
            // 上一份搜索结果
            const prevPluginResult = searchResultStore.getValue().data[plugin.hash];
            /** 上一份搜索还没返回/已经结束 */
            if ((prevPluginResult?.state ===
            ...
            /** 搜索的页码 */
            const page = queryPage ?? newSearch ? 1 : (prevPluginResult?.page ?? 0) + 1;
            try {
                searchResultStore.setValue(produce(draft => {
                ...
                const result = await plugin?.methods?.search?.(query, page, "lyric");
                /** 如果搜索结果不是本次结果 */
                ...
                devLog("error", "搜索失败", `Plugin: ${plugin.name} Query: ${query} Page: ${page}`, e, e?.message);
                /** 如果搜索结果不是本次结果 */
                if (currentQueryRef.current !== query) {
                    return;
                }
