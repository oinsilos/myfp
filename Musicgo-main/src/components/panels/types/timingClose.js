...
const shortCutTimes = [10, 20, 30, 45, 60];
function CountDownHeader() {
    const countDown = useScheduleCloseCountDown();
    const { t } = useI18N();
    return (<PanelHeader hideDivider hideButtons title={countDown === null
...
export default function TimingClose() {
    ...
    const { t } = useI18N();
    return (<PanelBase keyboardAvoidBehavior="none" positionMethod='top' height={rpx(450)} renderBody={() => (<>
                    <CountDownHeader />
                    <Divider />
                    <View style={styles.bodyContainer}>
                        {shortCutTimes.map((time, index) => (<TouchableOpacity style={styles.timeItem} key={index} activeOpacity={0.6} onPress={() => {
                    setScheduleClose(Date.now() + time * 60000);
                }}>
                ...
                setCloseAfterPlayEnd(!closeAfterPlay);
                        ...
                        {isCountingDown && (<TouchableOpacity style={styles.cancelButton} onPress={() => {
