...
export default function ColorPicker(props) {
    const { onSelected, defaultColor = "#66ccff", closePanelWhenSelected = true, } = props;
    const { t } = useI18N();
    const [currentHue, setCurrentHue] = useState(Color(defaultColor).hue());
    const [currentSaturation, setCurrentSaturation] = useState(Color(defaultColor).saturationl());
    const [currentLightness, setCurrentLightness] = useState(Color(defaultColor).lightness());
    const [currentAlpha, setCurrentAlpha] = useState(Color(defaultColor).alpha());
    const [inputValue, setInputValue] = useState(() => Color(defaultColor).rgb().hexa().toString());
    const hueColor = useMemo(() => Color.hsl(currentHue, 100, 50), [currentHue]);
    const currentColor = useMemo(() => Color.hsl(currentHue, currentSaturation, currentLightness), [currentHue, currentSaturation, currentLightness]);
    const currentColorWithAlpha = useMemo(() => currentColor.alpha(currentAlpha), [currentColor, currentAlpha]);
    ...
    const handleSLUpdate = useCallback((x, y) => {
    ...
    const handleHueUpdate = useCallback((y) => {
        const yRate = Math.min(1, Math.max(0, y / areaSize));
        setCurrentHue(yRate * 360);
    }, []);
    const handleAlphaUpdate = useCallback((y) => {
        const yRate = Math.min(1, Math.max(0, y / areaSize));
    ...
    const handleColorInputChange = useCallback((text) => {
        setInputValue(text);
    }, []);
    ...
    return (<PanelBase height={rpx(750)} keyboardAvoidBehavior="height" renderBody={() => (<>
                    <PanelHeader onCancel={hidePanel} onOk={async () => {
                // 检查输入框的值是否与当前颜色不同
                ...
                styles.slContainer,
                {
                    backgroundColor: hueColorString,
                                ...
                                <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={["#808080", "rgba(0,0,0,0)"]} style={[styles.slContainer, styles.layer1]}/>
                                <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} colors={["#fff", "rgba(0,0,0,0)", "#000"]} style={[styles.slContainer, styles.layer2]}/>
                                <View style={[
                styles.slThumb,
                slThumbStyle,
            ]}/>
                ...
                alphaThumbStyle,
                                ...
                                <Image resizeMode="repeat" source={ImgAsset.transparentBg} style={styles.transparentBg}/>
                            </LinearGradient>
                        </GestureDetector>
                    </View>
                ...
                styles.showBarContent,
                {
                    backgroundColor: currentColorWithAlphaString,
                        ...
                        <TextInput style={styles.colorInput} value={inputValue} onChangeText={handleColorInputChange} onSubmitEditing={handleColorInputSubmit} onBlur={handleColorInputBlur} placeholder="#RRGGBBAA" placeholderTextColor="#999" autoCapitalize="none" autoCorrect={false} returnKeyType="done"/>
                    </View>
                </>)}/>);
}
