...
export default function ImportMusicSheet() {
    ...
    return (<PanelBase height={vmax(60)} renderBody={() => (<>
                    <PanelHeader hideButtons title={t("panel.importMusicSheet.title")}/>
                    {validPlugins.length ? (<View style={globalStyle.fwflex1}>
                            <FlatList data={validPlugins} keyExtractor={plugin => plugin.hash} style={{
                    marginBottom: safeAreaInsets.bottom,
                }} renderItem={({ item: plugin }) => (<ListItem withHorizontalPadding key={`${plugin.hash}`} onPress={async () => {
                        showPanel("SimpleInput", {
                            ...
                            maxLength: 1000, async onOk(text, closePanel) {
                                Toast.success(t("panel.importMusicSheet.importing"));
                                closePanel();
                        ...
                        </View>) : (<NoPlugin notSupportType={t("panel.importMusicSheet.title")}/>)}
                </>)}/>);
}
