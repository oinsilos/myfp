...
export default function MusicQuality(props) {
    ...
    return (<PanelBase height={rpx(520)} renderBody={() => (<>
                    <PanelHeader title={i18n.t("panel.musicQuality.title", {
                type: type === "play"
                    ? i18n.t("common.play")
                ...
                style.body,
                {
                    marginBottom: safeAreaInsets.bottom,
                        ...
                        {qualityKeys.map(key => {
                return (<Fragment key={`frag-${key}`}>
                                    <Pressable key={`btn-${key}`} style={style.item} onPress={() => {
                        ...
                        ? `(${sizeFormatter(musicItem.qualities[key]
                            .size)})`
                        : ""}
                                        </ThemeText>
                                    </Pressable>
