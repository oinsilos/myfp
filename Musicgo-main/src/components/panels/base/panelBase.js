...
import { BackHandler, DeviceEventEmitter, KeyboardAvoidingView, Pressable, StyleSheet, } from "react-native";
...
export default function (props) {
    const { height = vh(60), renderBody, keyboardAvoidBehavior, positionMethod = "bottom", } = props;
    ...
    const useAnimatedBase = useMemo(() => (orientation === "horizontal" ? rpx(750) : height), [orientation]);
    ...
    useEffect(() => {
        ...
        const listenerSubscription = DeviceEventEmitter.addListener("hidePanel", (callback) => {
            if (callback) {
    ...
    useAnimatedReaction(() => snapPoint.value, (result, prevResult) => {
    ...
    const panelBody = (<Animated.View style={[
            style.wrapper,
    ...
    return (<>
            <Pressable style={style.maskWrapper} onPress={() => {
            snapPoint.value = withTiming(0, timingConfig);
        }}>
                <Animated.View style={[style.maskWrapper, style.mask, maskAnimated]}/>
            </Pressable>
            {keyboardAvoidBehavior === "none" ? (panelBody) : (<KeyboardAvoidingView style={style.kbContainer} behavior={keyboardAvoidBehavior || "position"}>
                    {panelBody}
