...
import { BackHandler, DeviceEventEmitter, Pressable, StyleSheet, } from "react-native";
...
export default function (props) {
    const { hasMask, containerStyle, children, animationType = "SlideToTop", } = props;
    ...
    const panelAnimated = useAnimatedStyle(() => {
        if (animationType === "SlideToTop") {
            return {
                transform: [
                    {
                        translateY: withTiming((1 - snapPoint.value) * windowHeight, timingConfig),
                    },
                ],
            };
        }
    ...
    return (<>
            {hasMask ? (<Pressable style={style.maskWrapper} onPress={() => {
                snapPoint.value = withTiming(0, timingConfig);
            }}>
                    <Animated.View style={[style.maskWrapper, style.mask, maskAnimated]}/>
                </Pressable>) : null}
            <Animated.View pointerEvents={hasMask ? "box-none" : undefined} style={[
            style.wrapper,
            !hasMask
                ? {
