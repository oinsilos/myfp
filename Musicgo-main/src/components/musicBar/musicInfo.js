...
function _BarMusicItem(props) {
    ...
    return (<Animated.View style={[
            ...
            <FastImage style={styles.artworkImg} source={musicItem.artwork} placeholderSource={ImgAsset.albumDefault}/>
            <Text ellipsizeMode="tail" accessible={false} style={styles.textWrapper} numberOfLines={1}>
                <ThemeText fontSize="content" fontColor="musicBarText">
                    {musicItem?.title}
                </ThemeText>
                {musicItem?.artist && (<ThemeText fontSize="description" color={Color(colors.musicBarText)
                .alpha(0.6)
                .toString()}>
                        {" "}
                        -{musicItem.artist}
...
const BarMusicItem = memo(_BarMusicItem, (prev, curr) => prev.musicItem === curr.musicItem &&
...
function skipMusicItem(direction) {
    if (direction === -1) {
...
export default function MusicInfo(props) {
    ...
    const panGesture = Gesture.Pan()
        ...
        if (!success) {
            // 还原到原始位置
            transformSharedValue.value = withTiming(0, timingConfig.animationFast);
        }
        else {
            // fling
            ...
            if (musicItemWidthValue.value) {
                ...
                if (Math.abs(rate) > 0.3) {
                    // 先判断距离
                    skip = vX > 0 ? 1 : -1;
                    transformSharedValue.value = withTiming(skip, timingConfig.animationFast, () => {
    ...
    return (<GestureDetector gesture={gesture}>
            <View style={musicInfoStyles.infoContainer} onLayout={e => {
            musicItemWidthValue.value = e.nativeEvent.layout.width;
        }}>
                <BarMusicItem transformSharedValue={transformSharedValue} musicItem={siblingMusicItems.prev} activeIndex={-1}/>
                <BarMusicItem transformSharedValue={transformSharedValue} musicItem={musicItem} activeIndex={0}/>
                <BarMusicItem transformSharedValue={transformSharedValue} musicItem={siblingMusicItems.next} activeIndex={1}/>
            </View>
        </GestureDetector>);
}
