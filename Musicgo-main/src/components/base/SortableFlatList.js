...
export default function SortableFlatList(props) {
    const { data, renderItem, itemHeight, itemJustifyContent, marginTop, activeBackgroundColor, onSortEnd, } = props;
    // 不要干扰原始数据
    ...
    // 列表整体的高度
    const listContentHeight = useMemo(() => itemHeight * data.length, [data, itemHeight]);
    function scrollToTarget(forceScroll = false) {
        ...
        const nextTarget = Math.sign(direction.value) *
            Math.max(Math.abs(direction.value), 0.3) *
            300 +
            contentOffsetYRef.current;
        // 当前到极限了
        if ((contentOffsetYRef.current <= 2 &&
        ...
        scrollingRef.current = true;
        // 超出区域
        targetOffsetYRef.current = Math.min(Math.max(0, nextTarget), listContentHeight - (layoutRef.current?.height ?? 0));
    ...
    return (<View style={globalStyle.fwflex1}>
            {/* 纯展示 */}
            <FakeFlatListItem ref={_ => (fakeItemRef.current = _)} backgroundColor={activeBackgroundColor} renderItem={renderItem} itemHeight={itemHeight} item={activeItem} itemJustifyContent={itemJustifyContent}/>
            <FlashList scrollEnabled={scrollEnabled} ref={_ => {
            listRef.current = _;
        }} onLayout={evt => {
            layoutRef.current = evt.nativeEvent.layout;
        }} data={_data} estimatedItemSize={itemHeight} scrollEventThrottle={16} onTouchStart={e => {
            if (activeRef.current !== -1) {
                // 相对于整个页面顶部的距离
            ...
            if (activeRef.current !== -1) {
                ...
                if (contentOffsetYRef.current !== -1) {
                    index = Math.round((contentOffsetYRef.current +
                        offsetRef.current) /
                        itemHeight);
                }
                else {
                ...
                if (activeRef.current !== index) {
                    ...
                    nData.splice(index, 0, activeItem);
                    onSortEnd?.(nData);
                    // 测试用，正式时移除掉
                    // _setData(nData);
                }
            ...
            if (activeRef.current !== -1 &&
                Math.abs(contentOffsetYRef.current -
                    targetOffsetYRef.current) < 2) {
                scrollToTarget(true);
            }
        }} renderItem={({ item, index }) => {
            return (<SortableFlatListItem setScrollEnabled={setScrollEnabled} activeRef={activeRef} renderItem={renderItem} item={item} index={index} setActiveItem={setActiveItem} itemJustifyContent={itemJustifyContent} itemHeight={itemHeight}/>);
        }}/>
        </View>);
}
function _SortableFlatListItem(props) {
    ...
    const styleRef = useRef(StyleSheet.create({
        viewWrapper: {
            height: itemHeight,
            width: "100%",
            flexDirection: "row",
    ...
    return (<View style={styleRef.current.viewWrapper}>
            {renderItem({ item, index })}
...
const SortableFlatListItem = memo(_SortableFlatListItem, (prev, curr) => prev.index === curr.index && prev.item === curr.item);
const FakeFlatListItem = forwardRef(function (props, ref) {
    const { itemHeight, renderItem, item, backgroundColor, itemJustifyContent } = props;
    const styleRef = useRef(StyleSheet.create({
        viewWrapper: {
            height: itemHeight,
            width: "100%",
