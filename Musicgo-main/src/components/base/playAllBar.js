...
export default function (props) {
    ...
    return (<View style={style.topWrapper}>
            <Pressable style={style.playAll} onPress={() => {
            if (musicList) {
                ...
                <Icon name="play-circle" style={style.playAllIcon} size={iconSizeConst.normal} color={colors.text}/>
                <ThemeText fontWeight="bold">{t("playAllBar.title")}</ThemeText>
            </Pressable>
            {canStar && musicSheet ? (<IconButton name={starred ? "heart" : "heart-outline"} sizeType={"normal"} color={starred ? "#e31639" : undefined} style={style.optionButton} onPress={async () => {
                if (!starred) {
            ...
            <IconButton name="folder-plus" sizeType={"normal"} style={style.optionButton} onPress={async () => {
            showPanel("AddToMusicSheet", {
                musicItem: musicList ?? [],
                newSheetDefaultName: sheetName,
            });
        }}/>
            <IconButton name="pencil-square" sizeType={"normal"} style={style.optionButton} onPress={async () => {
