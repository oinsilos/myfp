...
// 计算tip的位置和三角形位置
const calculateTipPosition = (childRect, tipWidth, tipHeight, position, triangleSize) => {
    const margin = rpx(12);
    let tipLeft = 0;
    let tipTop = 0;
    let triangleLeft = 0;
...
// 三角形组件
const Triangle = ({ size, color, style }) => (<View style={[
        {
            width: 0,
            height: 0,
...
const TipPortal = ({ content, position, childRect, backgroundColor, textColor, triangleSize, visible, onHide, autoHideDuration, }) => {
    ...
    if (tipDimensions.width === 0 || tipDimensions.height === 0) {
        ...
        return (<View style={[styles.tipContainer, styles.measurementContainer]} onLayout={handleLayout}>
                <Text style={[styles.tipText, { color: textColor }]}>{content}</Text>
            </View>);
    }
    const { tipLeft, tipTop, triangleLeft, triangleTop, triangleRotation } = calculateTipPosition(childRect, tipDimensions.width, tipDimensions.height, position, triangleSize);
...
export default function Tip({ children, content, position = "top", autoHideDuration = 3000, backgroundColor, textColor, triangleSize = rpx(12), }) {
    ...
    return (<>
            ...
            {visible && childRect && (<Portal>
                    <TipPortal content={content} position={position} childRect={childRect} backgroundColor={finalBackgroundColor} textColor={finalTextColor} triangleSize={triangleSize} visible={visible} onHide={handleHide} autoHideDuration={autoHideDuration}/>
                </Portal>)}
        </>);
}
