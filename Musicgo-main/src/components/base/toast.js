...
export function ToastBaseComponent() {
    ...
    useEffect(() => {
        if (activeToast) {
            toastAnim.value = withTiming(1, timingConfig.animationSlow, () => {
                toastAnim.value = withDelay(activeToast.duration || 1200, withTiming(0, timingConfig.animationSlow, finished => {
                    if (finished) {
    ...
    function removeCurrentToast() {
        if (toastAnim.value === 1) {
            ...
            toastAnim.value = withTiming(0, timingConfig.animationSlow, finished => {
                if (finished) {
                    runOnJS(setNextToast)();
                }
            });
    ...
    return activeToast ? (<GestureDetector gesture={flingGesture}>
            ...
            toastAnimStyle,
                    ...
                    <Icon size={fontSizeConst.appbar} name={typeConfig[activeToast.type].name} color={typeConfig[activeToast.type].color}/>
                    <Text numberOfLines={2} style={[styles.text, { color: colors.text }]}>
                        {activeToast.message}
                    </Text>
                    {activeToast.actionText && activeToast.onActionClick ? (<Pressable style={[
