...
export default function RadioDialog(props) {
    ...
    return (<Dialog onDismiss={hideDialog}>
            ...
            <FlatList ref={ref} style={{
            maxHeight: orientation === "horizontal" ? vmin(60) : vmax(60),
        }} data={content} getItemLayout={(_, index) => ({
            ...
            return (<ListItem withHorizontalPadding onPress={() => {
                    ...
                    hideDialog();
                            ...
                            {isConfig && item.icon ? (<ListItem.ListItemIcon icon={item.icon}/>) : null}
                            <ListItem.Content title={isConfig ? item.label : item}/>
                            {defaultSelected !== undefined &&
                    defaultSelected ===
                        (isConfig ? item.value : item) ? (<ListItem.ListItemIcon icon={"check"} color={colors.primary}/>) : null}
                        </ListItem>);
        }}/>
        </Dialog>);
}
