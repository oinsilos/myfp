 ...
 * @returns
 */
export function getMediaUniqueKey(mediaItem) {
 ...
 * @returns
 */
export function parseMediaUniqueKey(key) {
    try {
 ...
 * @returns
 */
export function isSameMediaItem(a, b) {
    // eslint-disable-next-line eqeqeq
    return !!(a && b && a.id == b.id && a.platform === b.platform);
}
/** 获取复位的mediaItem */
export function resetMediaItem(mediaItem, platform, newObj) {
    // 本地音乐不做处理
    if (mediaItem.platform === localPluginPlatform ||
 ...
 * @returns
 */
export function getLocalPath(mediaItem) {
    if (!mediaItem) {
        return null;
    }
    // 如果本身就是一个内部音乐
