...
export default class LyricParser {
    ...
    constructor(raw, options) {
        ...
        if (translation) {
            ...
            while (p1 < this.lrcItems.length) {
                ...
                while (transLrcItems[p2].time < lrcItem.time &&
    ...
    getPosition(position) {
        ...
        for (index = this.lastSearchIndex; index < this.lrcItems.length - 1; ++index) {
            if (position >= this.lrcItems[index].time &&
    ...
    toString(options) {
        ...
        if (withTimestamp) {
            return this.lrcItems
                .map(item => `${this.timeToLrctime(item.time)} ${type === "raw" ? item.lrc : item.translation}`)
                .join("\r\n");
        }
        else {
            return this.lrcItems
