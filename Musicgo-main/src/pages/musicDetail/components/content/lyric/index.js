...
export default function Lyric(props) {
    ...
    const [draggingIndex, setDraggingIndex, setDraggingIndexImmi] = useDelayFalsy(undefined, 2000);
    ...
    const blankComponent = useMemo(() => {
        return (<View style={styles.empty} onLayout={evt => {
                itemHeightsRef.current.blankHeight =
                    evt.nativeEvent.layout.height;
            }}/>);
    }, []);
    const handleLyricItemLayout = useCallback((index, height) => {
    ...
    return (<>
            <GestureDetector gesture={tapGesture}>
                <View style={globalStyle.fwflex1}>
                    {loading ? (<Loading color="white"/>) : lyrics?.length ? (<FlatList ref={_ => {
                listRef.current = _;
            }} onLayout={e => {
                ...
                delay(120).then(() => {
                    listRef.current?.scrollToIndex({
            ...
            }} fadingEdgeLength={120} ListHeaderComponent={<>
                                    {blankComponent}
                        ...
                        title: associateMusicItem.title || "",
                                                ...
                                                <GestureDetector gesture={unlinkTapGesture}>
                                                    <Text style={[
                        styles.linkText,
                        fontSizeStyle,
                    ]}>
                                ...
                                </>} ListFooterComponent={blankComponent} onScrollBeginDrag={onScrollBeginDrag} onMomentumScrollEnd={onScrollEndDrag} onScroll={onScroll} scrollEventThrottle={32} style={styles.wrapper} data={lyrics} initialNumToRender={30} overScrollMode="never" extraData={currentLrcItem} renderItem={({ item, index }) => {
                let text = item.lrc;
                if (showTranslation && hasTranslation) {
                    text += `\n${item?.translation ?? ""}`;
                }
                return (<LyricItemComponent index={index} text={text} fontSize={fontSizeStyle.fontSize} onLayout={handleLyricItemLayout} light={draggingIndex === index} highlight={currentLrcItem?.index === index}/>);
                ...
                showPanel("SearchLrc", {
                    musicItem: TrackPlayer.currentMusic,
                                ...
                                <Text style={[styles.searchLyric, fontSizeStyle]}>
                                    {t("lyric.searchLyric")}
                                </Text>
                            </TapGestureHandler>
                        </View>)}
                    {draggingIndex !== undefined && (<View style={[
                styles.draggingTime,
                layout?.height
                    ? {
                    ...
                    : null,
                            ...
                            <DraggingTime time={(lyrics[draggingIndex]?.time ?? 0) +
                +(meta?.offset ?? 0)}/>
                            <View style={styles.singleLine}/>

                            <IconButtonWithGesture style={styles.playIcon} sizeType='normal' name="play" onPress={onLyricSeekPress}/>
                        </View>)}
                </View>
            </GestureDetector>
