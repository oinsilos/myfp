...
export default function (pluginHash, tag) {
    ...
    const query = useCallback(async () => {
        ...
        try {
            ...
            if (plugin) {
                ...
                const res = await plugin.methods?.getRecommendSheetsByTag?.(tag, pageRef.current);
                if (res.isEnd) {
                ...
                if (tag.id === currentTagRef.current) {
                    setSheets(prev => [
                        ...prev,
                        ...res.data.map(item => resetMediaItem(item, plugin.instance.platform)),
                    ]);
                }
            }
            else {
    ...
    return [query, sheets, requestState];
}
