...
function SheetBody(props) {
    ...
    // 第一个tag
    const [firstTag, setFirstTag] = useState(defaultTag);
    // 所有tag
    const tags = useRecommendList(hash);
    return (<View style={globalStyle.fwflex1}>
            <ScrollView style={style.headerWrapper} contentContainerStyle={style.header} showsHorizontalScrollIndicator={false} horizontal>
                <TypeTag title={firstTag.title} selected={selectedTag.id === firstTag.id} onPress={() => {
            // 拉起浮层
            showPanel("SheetTags", {
                tags: tags?.data ?? [],
                onTagPressed(tag) {
                ...
                {(tags?.pinned ?? []).map(_ => (<TypeTag key={`pinned-${_.id}`} title={_?.title ?? ""} selected={selectedTag.id === _.id} onPress={() => {
                setSelectedTag(_);
            }}/>))}
            </ScrollView>
