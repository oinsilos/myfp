...
export default function useAlbumDetail(originalAlbumItem) {
    const currentPageRef = useRef(1);
    const [requestState, setRequestState] = useState(RequestStateCode.IDLE);
    const [albumItem, setAlbumItem] = useState(originalAlbumItem);
    const [musicList, setMusicList] = useState(originalAlbumItem?.musicList ?? []);
    const getAlbumDetail = useCallback(async function () {
        ...
        try {
            ...
            const result = await PluginManager.getByMedia(originalAlbumItem)?.methods?.getAlbumInfo?.(originalAlbumItem, currentPageRef.current);
            if (!result) {
                throw new Error();
            }
            if (result?.albumItem) {
    ...
    return [requestState, albumItem, musicList, getAlbumDetail];
}
