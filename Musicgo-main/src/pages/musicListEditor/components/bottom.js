...
export default function Bottom() {
    ...
    function resetSelectedIndices() {
        setEditingMusicList(editingMusicList.map(_ => ({
            musicItem: _.musicItem,
            checked: false,
        })));
    }
    return (<View style={style.wrapper}>
            <BottomIcon icon="motion-play" title={t("musicListEditor.addToNextPlay")} onPress={async () => {
            TrackPlayer.addNext(selectedItems);
            resetSelectedIndices();
            Toast.success(t("toast.addToNextPlay"));
        }}/>
            <BottomIcon icon="folder-plus" title={t("musicListEditor.addToSheet")} onPress={() => {
            ...
            <BottomIcon icon="arrow-down-tray" title={t("common.download")} onPress={() => {
            if (selectedItems.length) {
            ...
            <BottomIcon icon="trash-outline" title={t("common.delete")} color={selectedItems.length && musicSheet?.id
            ? "text"
            : "textSecondary"} onPress={() => {
...
function BottomIcon(props) {
    ...
    return (<Pressable onPress={onPress} style={[style.bottomIconWrapper, { backgroundColor: colors.appBar }]}>
            <Icon name={icon} color={colors.appBarText} style={color === "textSecondary" ? style.opacity_06 : undefined} size={iconSizeConst.big} onPress={onPress}/>
            <ThemeText fontSize="subTitle" fontColor={"appBarText"} opacity={color === "textSecondary" ? 0.6 : undefined} style={style.bottomIconText}>
                {title}
            </ThemeText>
        </Pressable>);
}
