...
export default function useQueryArtist(pluginHash) {
    const [queryResults, setQueryResults] = useAtom(queryResultAtom);
    const queryArtist = useCallback(async (artist, page, type = "music") => {
        const plugin = PluginManager.getByHash(pluginHash);
        const prevResult = queryResults[type];
        if (prevResult?.state === RequestStateCode.PENDING_FIRST_PAGE ||
        ...
        try {
            setQueryResults(produce(draft => {
                draft[type].state = page === 1 ? RequestStateCode.PENDING_FIRST_PAGE : RequestStateCode.PENDING_REST_PAGE;
            }));
            const result = await plugin?.methods?.getArtistWorks?.(artist, page, type);
            setQueryResults(produce(draft => {
                ...
                if (page === 1) {
                    ...
                    draft[type].data = (draft[type].data ?? []).concat(result?.data ?? []);
                }
            }));
        }
        catch (e) {
