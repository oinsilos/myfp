...
export default function useTopListDetail(topListItem, pluginHash) {
    const [mergedTopListItem, setMergedTopListItem] = useState(topListItem);
    ...
    async function loadMore() {
        ...
        try {
            if (requestState === RequestStateCode.PENDING_FIRST_PAGE ||
            ...
            const result = await PluginManager.getByHash(pluginHash)?.methods?.getTopListDetail(topListItem, pageRef.current);
            if (!result) {
                throw new Error();
            }
    ...
    return [mergedTopListItem, requestState, loadMore];
}
