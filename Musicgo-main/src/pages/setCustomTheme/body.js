...
export default function Body() {
    ...
    async function onImageClick() {
        try {
            ...
            const bgPath = `${pathConst.dataPath}background${uri.substring(uri.lastIndexOf("."))}`;
            await copyFile(uri, bgPath);
            const colorsResult = await ImageColors.getColors(uri, {
                fallback: "#ffffff",
            });
            const colors = {
                primary: colorsResult.platform === "android"
                    ? colorsResult.dominant
                    : colorsResult.platform === "ios"
                        ? colorsResult.primary
                        : colorsResult.vibrant,
                average: colorsResult.platform === "android"
                    ? colorsResult.average
                    : colorsResult.platform === "ios"
                        ? colorsResult.detail
                        : colorsResult.dominant,
                vibrant: colorsResult.platform === "android"
                    ? colorsResult.vibrant
                    : colorsResult.platform === "ios"
            ...
            if (primaryGrayRate < -0.4) {
                ...
                console.log(colors.primary, primaryGrayRate, primaryColor
    ...
    return (<ScrollView style={globalStyle.fwflex1}>
            <TouchableOpacity onPress={onImageClick}>
                <Image style={styles.image} uri={backgroundInfo?.url} emptySrc={ImgAsset.addBackground}/>
            </TouchableOpacity>

            <View style={styles.sliderWrapper}>
                <ThemeText>{t("setCustomTheme.blur")}</ThemeText>
                <Slider style={styles.slider} minimumTrackTintColor={theme.colors.primary} maximumTrackTintColor={theme.colors.text ?? "#999999"} thumbTintColor={theme.colors.primary} minimumValue={0} step={1} maximumValue={30} onSlidingComplete={val => {
            Theme.setBackground({
                blur: val,
            });
            ...
            <View style={styles.sliderWrapper}>
                ...
                <Slider style={styles.slider} minimumTrackTintColor={theme.colors.primary} maximumTrackTintColor={theme.colors.text ?? "#999999"} thumbTintColor={theme.colors.primary} minimumValue={0.3} step={0.01} maximumValue={1} onSlidingComplete={val => {
            Theme.setBackground({
                opacity: val,
            });
            ...
            <View style={styles.colorsContainer}>
                {Theme.configableColorKey.map(key => (<View key={key} style={styles.colorItem}>
                        <ThemeText>{t("setCustomTheme." + key + "Color")}</ThemeText>
                ...
                styles.colorBlock,
                            ...
                            <ThemeText fontSize="subTitle" style={styles.colorText}>
                                {
            /** @ts-ignore */
