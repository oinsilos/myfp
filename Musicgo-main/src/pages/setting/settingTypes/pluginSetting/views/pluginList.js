...
export default function PluginList() {
    ...
    async function onInstallFromLocalClick() {
        try {
            ...
            setLoading(true);
            await Promise.all(results.assets.map(async (it) => {
    ...
    return (<>
            ...
            <HorizontalSafeAreaView style={style.wrapper}>
                <>
                    {loading ? (<Loading />) : (<FlatList ListEmptyComponent={Empty} ListFooterComponent={<View style={style.blank}/>} data={plugins ?? []} keyExtractor={_ => _.hash} renderItem={({ item: plugin }) => (<PluginItem key={plugin.hash} plugin={plugin}/>)}/>)}
...
async function installPluginFromUrl(text) {
    try {
        let urls = [];
        const inputUrl = text.trim();
        if (text.endsWith(".json")) {
            const jsonFile = (await axios.get(inputUrl, {
                headers: {
             ...
             * }
             */
            urls = (jsonFile?.plugins ?? []).map((_) => _.url);
        }
        else {
            urls = [inputUrl];
        }
        return await Promise.all(urls.map(url => PluginManager.installPluginFromUrl(url, {
            notCheckVersion: Config.getConfig("basic.notCheckPluginVersion"),
        })));
    }
    catch (e) {
