...
export default function BackupSetting() {
            ...
            async onAction(selectedFiles) {
                ...
                return new Promise(resolve => {
                    showDialog("LoadingDialog", {
                        ...
                        promise: writeInChunks(`${folder}${folder?.endsWith("/") ? "" : "/"}backup.json`, raw),
    ...
    async function onResumeFromWebdav() {
        ...
        try {
            const resumeData = await client.getFileContents("/MusicFree/MusicFreeBackup.json", {
                format: "text",
            });
            await Backup.resume(resumeData, Config.getConfig("backup.resumeMode"));
            Toast.success(t("toast.resumeSuccess"));
        }
        catch (e) {
            Toast.warn(t("toast.resumeFail", { reason: e?.message ?? e }));
    ...
    async function onBackupToWebdav() {
        ...
        try {
            ...
            // 临时文件
            await client.putFileContents("/MusicFree/MusicFreeBackup.json", raw, {
                overwrite: true,
            });
            Toast.success(t("toast.backupSuccess"));
        }
    ...
    return (<ScrollView style={style.wrapper}>
            <ListItemHeader>{t("sidebar.backupAndResume")}</ListItemHeader>

            <ListItem withHorizontalPadding onPress={() => {
            showDialog("RadioDialog", {
                title: t("backupAndResume.setResumeMode"),
                content: [
                    {
                        label: t(("backupAndResume.resumeMode." + "append" /* ResumeMode.Append */)),
                        value: "append" /* ResumeMode.Append */,
                    },
                    {
                        label: t(("backupAndResume.resumeMode." + "overwrite-default" /* ResumeMode.OverwriteDefault */)),
                        value: "overwrite-default" /* ResumeMode.OverwriteDefault */,
                    },
                    {
                        label: t(("backupAndResume.resumeMode." + "overwrite" /* ResumeMode.Overwrite */)),
                        value: "overwrite" /* ResumeMode.Overwrite */,
                    },
                ],
                onOk(value) {
                    Config.setConfig("backup.resumeMode", value);
                },
            });
        }}>
                <ListItem.Content title={t("backupAndResume.resumeMode")}/>
                <ListItem.ListItemText>
                    {t(("backupAndResume.resumeMode." + (resumeMode || "append" /* ResumeMode.Append */)))}
                </ListItem.ListItemText>
            </ListItem>
