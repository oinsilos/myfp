...
export default function useSearch() {
     ...
     * pluginHash: 搜索条件
     */
    const search = useCallback(async function (query, queryPage, type, pluginHash) {
        /** 如果没有指定插件，就用所有插件搜索 */
        let plugins = [];
        if (pluginHash) {
        ...
        plugins.forEach(async (plugin) => {
            ...
            /** 搜索的页码 */
            const page = queryPage ?? newSearch
                ? 1
            ...
            try {
                ...
                /** 切换到结果页 */
                const currentPageStatus = getDefaultStore().get(pageStatusAtom);
