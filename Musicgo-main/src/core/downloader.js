...
(function (DownloadStatus) {
    // 等待下载
    DownloadStatus[DownloadStatus["Pending"] = 0] = "Pending";
    // 准备下载链接
    DownloadStatus[DownloadStatus["Preparing"] = 1] = "Preparing";
    // 下载中
    DownloadStatus[DownloadStatus["Downloading"] = 2] = "Downloading";
    // 下载完成
    DownloadStatus[DownloadStatus["Completed"] = 3] = "Completed";
    // 下载失败
    DownloadStatus[DownloadStatus["Error"] = 4] = "Error";
})(DownloadStatus || (DownloadStatus = {}));
export var DownloaderEvent;
(function (DownloaderEvent) {
    // 某次下载行为出错
    DownloaderEvent["DownloadError"] = "download-error";
    // 下载任务更新
    DownloaderEvent["DownloadTaskUpdate"] = "download-task-update";
    // 下载某个音乐时出错
    DownloaderEvent["DownloadTaskError"] = "download-task-error";
    // 下载完成
    DownloaderEvent["DownloadQueueCompleted"] = "download-queue-completed";
})(DownloaderEvent || (DownloaderEvent = {}));
export var DownloadFailReason;
(function (DownloadFailReason) {
    /** 无网络 */
    DownloadFailReason["NetworkOffline"] = "network-offline";
    /** 设置-禁止在移动网络下下载 */
    DownloadFailReason["NotAllowToDownloadInCellular"] = "not-allow-to-download-in-cellular";
    /** 无法获取到媒体源 */
    DownloadFailReason["FailToFetchSource"] = "no-valid-source";
    /** 没有文件写入的权限 */
    DownloadFailReason["NoWritePermission"] = "no-write-permission";
    DownloadFailReason["Unknown"] = "unknown";
...
class Downloader extends EventEmitter {
    ...
    downloadingCount = 0;
    static generateFilename(musicItem) {
    ...
    injectDependencies(configService, pluginManager) {
        this.configService = configService;
        this.pluginManagerService = pluginManager;
    }
    updateDownloadTask(musicItem, patch) {
        const newValue = {
    ...
    // 开始下载
    markTaskAsStarted(musicItem) {
        this.downloadingCount++;
        this.updateDownloadTask(musicItem, {
            status: DownloadStatus.Preparing,
        });
    }
    markTaskAsCompleted(musicItem) {
        this.downloadingCount--;
        this.updateDownloadTask(musicItem, {
            status: DownloadStatus.Completed,
        });
    }
    markTaskAsError(musicItem, reason, error) {
        this.downloadingCount--;
        this.updateDownloadTask(musicItem, {
    ...
    async downloadNextPendingTask() {
        ...
        try {
            if (plugin) {
                const qualityOrder = getQualityOrder(nextTask.quality ??
                    this.configService.getConfig("basic.defaultDownloadQuality") ??
                    "standard", this.configService.getConfig("basic.downloadQualityOrder") ?? "asc");
                let data = null;
        ...
        // 缓存下载地址
        const cacheDownloadPath = addFileScheme(this.getCacheDownloadPath(`${nanoid()}.${extension}`));
        // 真实下载地址
        const targetDownloadPath = addFileScheme(this.getDownloadPath(`${nextTask.filename}.${extension}`));
        // 检测下载位置是否存在
        try {
...
export function useDownloadTask(musicItem) {
