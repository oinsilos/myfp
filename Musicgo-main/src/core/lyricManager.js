...
class LyricManager {
    ...
    injectDependencies(trackPlayerService, appConfigService, pluginManager) {
    ...
    setup() {
        ...
        this.trackPlayer.on(TrackPlayerEvents.CurrentMusicChanged, (musicItem) => {
            ...
            if (this.appConfig.getConfig("lyric.showStatusBarLyric")) {
                if (musicItem) {
                    LyricUtil.setStatusBarLyricText(`${musicItem.title} - ${musicItem.artist}`);
                }
                else {
        ...
        RNTrackPlayer.addEventListener(Event.PlaybackProgressUpdated, evt => {
            ...
            if (currentLyricItem?.lrc !== newLyricItem?.lrc) {
                ...
                if (this.appConfig.getConfig("lyric.showStatusBarLyric")) {
                    LyricUtil.setStatusBarLyricText((newLyricItem?.lrc ?? "") +
                        (showTranslation
    ...
    associateLyric(musicItem, linkToMusicItem) {
    ...
    async uploadLocalLyric(musicItem, lyricContent, type = "raw") {
        if (!musicItem) {
            return;
        }
    ...
    async removeLocalLyric(musicItem) {
        if (!musicItem) {
            return;
        }
        const platformHash = CryptoJs.MD5(musicItem.platform).toString(CryptoJs.enc.Hex);
    ...
    updateLyricOffset(musicItem, offset) {
        if (!musicItem) {
            return;
        }
        // 更新歌词偏移
    ...
    async refreshLyric(skipFetchLyricSourceIfSame = true, ignoreProgress = false) {
        ...
        try {
            ...
            if (this.appConfig.getConfig("lyric.showStatusBarLyric")) {
                if (currentLyric) {
                    LyricUtil.setStatusBarLyricText((currentLyric?.lrc ?? "") +
     ...
     * @returns
     */
    async searchSimilarLyric(musicItem) {
