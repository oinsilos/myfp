...
/** 获取meta信息 */
const getMediaCache = (mediaItem) => {
    if (mediaItem.platform && mediaItem.id) {
        const cacheMediaItem = mediaCacheStore.getString(getMediaUniqueKey(mediaItem));
...
async function clearLocalCaches(cacheData) {
...
async function checkPathAndRemove(filePath) {
    if (!filePath) {
        return;
    }
...
/** 移除缓存信息 */
const removeMediaCache = (mediaItem) => {
    if (mediaItem.platform && mediaItem.id) {
