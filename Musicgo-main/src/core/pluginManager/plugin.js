...
function formatAuthUrl(url) {
    ...
    try {
        if (urlObj.username && urlObj.password) {
            const auth = `Basic ${Base64.btoa(`${decodeURIComponent(urlObj.username)}:${decodeURIComponent(urlObj.password)}`)}`;
            urlObj.username = "";
            urlObj.password = "";
            return {
                url: urlObj.toString(),
...
(function (PluginState) {
    // 初始化
    PluginState[PluginState["Initializing"] = 0] = "Initializing";
    // 加载中
    PluginState[PluginState["Loading"] = 1] = "Loading";
    // 已加载
    PluginState[PluginState["Mounted"] = 2] = "Mounted";
    // 出现错误
    PluginState[PluginState["Error"] = 3] = "Error";
})(PluginState || (PluginState = {}));
export var PluginErrorReason;
(function (PluginErrorReason) {
    // 版本不匹配
    PluginErrorReason[PluginErrorReason["VersionNotMatch"] = 0] = "VersionNotMatch";
    // 无法解析
    PluginErrorReason[PluginErrorReason["CannotParse"] = 1] = "CannotParse";
})(PluginErrorReason || (PluginErrorReason = {}));
class PluginMethodsWrapper {
    plugin;
    ensurePluginIsMounted;
    ...
    /** 获取真实源 */
    async getMediaSource(musicItem, quality = "standard", retryCount = 1, notUpdateCache = false) {
        await this.ensurePluginIsMounted();
        ...
        if (!parserPlugin.instance.getMediaSource) {
            const { url, auth } = formatAuthUrl(musicItem?.qualities?.[quality]?.url ?? musicItem.url);
            return {
                url: url,
                headers: auth
                    ? {
        ...
        try {
            const { url, headers } = (await parserPlugin.instance.getMediaSource(musicItem, quality)) ?? { url: musicItem?.qualities?.[quality]?.url };
            if (!url) {
    ...
    async getMusicInfo(musicItem) {
        ...
        try {
            return (this.plugin.instance.getMusicInfo(resetMediaItem(musicItem, undefined, true)) ?? null);
        }
        catch (e) {
     ...
     */
    /** 获取歌词 */
    async getLyric(originalMusicItem) {
        await this.ensurePluginIsMounted();
        // 1.额外存储的meta信息（关联歌词）
        const associatedLrc = getMediaExtraProperty(originalMusicItem, "associatedLrc");
        let musicItem;
        if (associatedLrc) {
            musicItem = associatedLrc;
        }
        else {
            musicItem = originalMusicItem;
        }
        const musicItemCache = MediaCache.getMediaCache(musicItem);
        /** 原始歌词文本 */
        let rawLrc = musicItem.rawLrc || null;
        ...
        if (await RNFS.exists(pathConst.localLrcPath + platformHash + "/" + idHash + ".lrc")) {
            rawLrc = await RNFS.readFile(pathConst.localLrcPath + platformHash + "/" + idHash + ".lrc", "utf8");
            if (await RNFS.exists(pathConst.localLrcPath +
                platformHash +
                "/" +
                idHash +
                ".tran.lrc")) {
                translation =
                    (await RNFS.readFile(pathConst.localLrcPath +
                        platformHash +
                        "/" +
                        idHash +
                        ".tran.lrc", "utf8")) || null;
        ...
        if (musicItemCache?.lyric) {
            // 缓存的远程结果
            let cacheLyric = musicItemCache.lyric || null;
            // 缓存的本地结果
            let localLyric = musicItemCache.$localLyric || null;
            // 优先用缓存的结果
            ...
            if (localLyric) {
                ...
                if (localLyric.translation &&
                    (await exists(localLyric.translation))) {
                    translation = await readFile(localLyric.translation, "utf8");
                }
        ...
        if (lrcSource) {
            ...
            // 本地的文件名
            let filename = `${pathConst.lrcCachePath}${nanoid()}.lrc`;
            let filenameTrans = `${pathConst.lrcCachePath}${nanoid()}.lrc`;
            // 旧版本兼容
            ...
            if (rawLrc || translation) {
                MediaCache.setMediaCache(produce(musicItemCache || musicItem, draft => {
                    musicItemCache?.$localLyric?.rawLrc;
                    objectPath.set(draft, "$localLyric.rawLrc", filename);
                    objectPath.set(draft, "$localLyric.translation", filenameTrans);
                    return draft;
                }));
                return {
                    rawLrc: rawLrc || undefined,
    ...
    async getAlbumInfo(albumItem, page = 1) {
        ...
        if (!this.plugin.instance.getAlbumInfo) {
            return {
                albumItem,
                musicList: (albumItem?.musicList ?? []).map(resetMediaItem, this.plugin.name, true),
                isEnd: true,
            };
        }
        try {
            const result = await this.plugin.instance.getAlbumInfo(resetMediaItem(albumItem, undefined, true), page);
            if (!result) {
                throw new Error();
            }
    ...
    async getMusicSheetInfo(sheetItem, page = 1) {
        ...
        try {
            const result = await this.plugin.instance?.getMusicSheetInfo?.(resetMediaItem(sheetItem, undefined, true), page);
            if (!result) {
                throw new Error();
            }
            result?.musicList?.forEach(_ => {
    ...
    async getArtistWorks(artistItem, page, type) {
        ...
        try {
            const result = await this.plugin.instance.getArtistWorks(artistItem, page, type);
            if (!result.data) {
                return {
                    isEnd: true,
                    data: [],
    ...
    async getRecommendSheetsByTag(tagItem, page) {
        await this.ensurePluginIsMounted();
        try {
            const result = await this.plugin.instance?.getRecommendSheetsByTag?.(tagItem, page ?? 1);
            if (!result) {
                throw new Error();
            }
            if (result.isEnd !== false) {
    ...
    async getMusicComments(musicItem, page) {
        await this.ensurePluginIsMounted();
        const result = await this.plugin.instance?.getMusicComments?.(musicItem, page ?? 1);
        if (!result) {
            throw new Error();
        }
        if (result.isEnd !== false) {
...
export class Plugin {
    ...
    static injectDependencies(pluginManager) {
        Plugin.pluginManager = pluginManager;
    }
    constructor(funcCode, pluginPath, lazyProps = null) {
        ...
        if (!lazyProps) {
            ...
            this.supportedMethods = new Set((lazyProps.supportedMethods ?? []));
            // 初始化方法，但实际调用时会先挂载插件
    ...
    mountPlugin(funcCode, pluginPath) {
        ...
        try {
            if (typeof funcCode === "string") {
                ...
                _instance = evalSandbox.load(funcCode, createRnPluginHost({
                    getUserVariables: () => _internalPluginMeta.getUserVariables(this.name),
                }));
            }
            else {
                _instance = funcCode();
        ...
        this.supportedMethods = new Set(Object.keys(_instance).filter(key => typeof (_instance[key]) === "function"));
        // 检测name & 计算hash
        if (this.name === "" ||
            !this.name) {
            this.hash = "";
...
const localFilePluginDefine = {
    ...
    async importMusicItem(urlLike) {
        ...
        try {
            ...
            id =
                CryptoJs.MD5(fileStat.originalFilepath).toString(CryptoJs.enc.Hex) || nanoid();
        }
        catch {
            id = nanoid();
        }
