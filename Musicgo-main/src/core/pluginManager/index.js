...
class PluginManager {
    ...
    updatePluginCache(plugin) {
        if (plugin.path && plugin.state === PluginState.Mounted) {
            pluginCacheStore.set(plugin.path, JSON.stringify({
                name: plugin.name,
                hash: plugin.hash,
                path: plugin.path,
        ...
        try {
            ...
            for (let i = 0; i < pluginsFileItems.length; ++i) {
                ...
                if (pluginFileItem.isFile() &&
                    ...
                    if (this.appConfigService.getConfig("basic.lazyLoadPlugin") &&
                        pluginCacheStore.contains(pluginFileItem.path)) {
                        isLazyLoad = true;
                        const lazyProps = safeParse(pluginCacheStore.getString(pluginFileItem.path));
                        lazyProps.loadFuncCode = async () => await readFile(pluginFileItem.path, "utf8");
     ...
     */
    async installPluginFromLocalFile(pluginPath, config) {
        let funcCode;
        if (config?.useExpoFs) {
    ...
    async installPluginFromUrl(url, config) {
        try {
            const funcCode = (await axios.get(url, {
                headers: {
    ...
    getSortedPlugins() {
        ...
        return [...this.getPlugins()].sort((a, b) => (order[a.name] ?? Infinity) - (order[b.name] ?? Infinity) < 0
            ? -1
            : 1);
    }
    /**
     ...
     * @returns 可搜索的插件实例数组
     */
    getSearchablePlugins(supportedSearchType) {
        return this.getPlugins().filter(it => pluginMeta.isPluginEnabled(it.name) &&
     ...
     */
    getSortedSearchablePlugins(supportedSearchType) {
        const order = pluginMeta.getPluginOrder();
        return [...this.getSearchablePlugins(supportedSearchType)].sort((a, b) => (order[a.name] ?? Infinity) - (order[b.name] ?? Infinity) < 0
            ? -1
            : 1);
    }
    /**
     ...
     */
    getSortedPluginsWithAbility(ability) {
        const order = pluginMeta.getPluginOrder();
        return [...this.getPluginsWithAbility(ability)].sort((a, b) => (order[a.name] ?? Infinity) - (order[b.name] ?? Infinity) < 0
            ? -1
            : 1);
    }
    /**
...
export function useSortedPlugins() {
    ...
    const [sortedPlugins, setSortedPlugins] = useState(pluginManager.getSortedPlugins());
    useEffect(() => {
        const callback = () => {
            const order = pluginMeta.getPluginOrder();
            setSortedPlugins([...plugins].sort((a, b) => (order[a.name] ?? Infinity) - (order[b.name] ?? Infinity) <
                0
                ? -1
                : 1));
        };
...
export function usePluginEnabled(plugin) {
    const [enabled, setEnabled] = useState(pluginManager.isPluginEnabled(plugin));
    useEffect(() => {
        const callback = (pluginName, _enabled) => {
