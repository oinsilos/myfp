...
export interface ICreateRnPluginHostOptions {
    /** 当前插件的用户变量读取函数（按插件名绑定） */
 ...
 */
export function createRnPluginHost(
    options: ICreateRnPluginHostOptions,
): IPluginHost {
    const requireFn = (packageName: string) => {
    ...
    const consoleObj: IPluginHostConsole = {
    ...
    const processObj: IPluginHostProcess = {
        platform: "android",
        version: appVersion,
        env,
    };
